...
var main = function() {
  ...
  lasers.forEach(function(a, b){ships.forEach(function(c, d) {if (a.x < c.x + c.data.sizeX.replace('px', '') && a.x + a.data.sizeX.replace('px', '') > c.x && a.y < c.y + c.data.sizeY.replace('px', '') && a.y + a.data.sizeY.replace('px', '') > c.y && a.age > 3) {a.remove(); lasers.splice(b, 1);c.remove(); ships.splice(d, 1);}})});
};
